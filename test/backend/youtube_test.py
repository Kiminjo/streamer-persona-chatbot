...
def list_broadcasts(youtube, broadcast_status):
   print('Broadcasts with status "%s":' % broadcast_status)
    ...
    livechatid = list_broadcasts_response['items'][0]['snippet']['liveChatId']
    ...
    chat_response = youtube.liveChatMessages().list(
        ...
        pageToken='GN6h5ayO_oUDINjPqbqQ_oUD',
    ).execute()

    nextpagetoken = chat_response['nextPageToken']

    for chat in chat_response.get('items', []):
        print((chat['snippet']['textMessageDetails']['messageText']))
